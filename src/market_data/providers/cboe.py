...
def _get_optionable_tickers_impl(**kwargs: Any) -> list[Ticker]:
    ...
    if not url:
        logging.error(f"Invalid CBOE symbol type: {symbol_type}. Valid types: {list(_CBOE_URLS.keys())}")
        return []
    ...
    try:
        ...
        if "Stock Symbol" not in df.columns:
            logging.error(f"CBOE CSV missing required 'Stock Symbol' column. Available columns: {list(df.columns)}")
            return []
        ...
        logging.info(f"Successfully parsed {len(valid_tickers)} optionable tickers from CBOE {symbol_type} data")
